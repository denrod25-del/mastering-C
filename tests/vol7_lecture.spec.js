...
const URL = '/lectures/vol7_lecture.html';

test.describe('Vol VII — Systems Programming player', () => {
  test('loads, renders the first slide, and throws no console/page errors', async ({ page }) => {
    ...
    await page.goto(URL);
    await expect(page.locator('#st')).toHaveText('The syscall interface -- user space to kernel');
  ...
  test('keyboard shortcuts do not fire while typing in the Ask text field', async ({ page }) => {
    ...
    await page.fill('#vp-input', 'what is a file descriptor?'); // contains a space char
