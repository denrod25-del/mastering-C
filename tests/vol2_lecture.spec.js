...
const URL = '/lectures/vol2_lecture.html';

test.describe('Vol II — Pointers Deep Dive player', () => {
  test('loads, renders the first slide, and throws no console/page errors', async ({ page }) => {
    ...
    await page.goto(URL);
    await expect(page.locator('#stitle')).toHaveText('The virtual address space');
    await expect(page.locator('#scount')).toHaveText('1 / 8');
  ...
  test('heading order and landmark are correct', async ({ page }) => {
    ...
    await expect(page.locator('#stitle')).toHaveJSProperty('tagName', 'H1');
  });
  ...
  test('play/pause toggles with correct labels (no raw HTML-entity text)', async ({ page }) => {
    ...
    expect(await page.locator('#playbtn').textContent()).not.toMatch(/&#/);
    await page.click('#playbtn');
  ...
  test('Prev/Next stay in bounds and disable at the edges', async ({ page }) => {
    ...
    await expect(page.locator('#scount')).toHaveText('8 / 8');
  ...
  test('keyboard shortcuts (Space, arrows) work outside inputs', async ({ page }) => {
    ...
    await expect(page.locator('#scount')).toHaveText('2 / 8');
  ...
  test('keyboard shortcuts do not fire while typing in the Ask text field', async ({ page }) => {
    ...
    await page.fill('#vp-input', 'what is the spiral rule?'); // contains a space char
    await expect(page.locator('#scount')).toHaveText('1 / 8'); // unchanged — space didn't trigger play
  });
  ...
  test('progress bar seeking is mouse- and keyboard-accessible', async ({ page }) => {
    ...
    const midSlide = await page.locator('#scount').textContent();
    expect(midSlide).toMatch(/^[3-6] \/ 8$/);
