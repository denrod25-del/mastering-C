...
const URL = '/lectures/vol3_lecture.html';

test.describe('Vol III — Types & Data Representation player', () => {
  test('loads, renders the first slide, and throws no console/page errors', async ({ page }) => {
    ...
    await page.goto(URL);
    await expect(page.locator('#st')).toHaveText('Integer types and the stdint.h contract');
  ...
  test('keyboard shortcuts do not fire while typing in the Ask text field', async ({ page }) => {
    ...
    await page.fill('#vp-input', 'what is a segfault?'); // contains a space char
