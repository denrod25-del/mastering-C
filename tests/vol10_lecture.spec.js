...
const URL = '/lectures/vol10_lecture.html';

test.describe('Vol X — Data Structures player', () => {
  test('loads, renders the first slide, and throws no console/page errors', async ({ page }) => {
    ...
    await page.goto(URL);
    await expect(page.locator('#st')).toHaveText('Dynamic array -- O(1) amortized append');
  ...
  test('keyboard shortcuts do not fire while typing in the Ask text field', async ({ page }) => {
    ...
    await page.fill('#vp-input', 'what is a hash collision?'); // contains a space char
