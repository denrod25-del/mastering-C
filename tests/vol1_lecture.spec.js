...
const URL = '/lectures/vol1_lecture.html';

test.describe('Vol I — Mastering Pointers player', () => {
  test('loads, renders the first slide, and throws no console/page errors', async ({ page }) => {
    ...
    await page.goto(URL);
    await expect(page.locator('#stitle')).toHaveText('What is a pointer?');
    await expect(page.locator('#scount')).toHaveText('1 / 10');
  ...
  test('Prev/Next stay in bounds and disable at the edges', async ({ page }) => {
    ...
    await expect(page.locator('#scount')).toHaveText('10 / 10');
  ...
  test('keyboard shortcuts (Space, arrows) work outside inputs', async ({ page }) => {
    ...
    await expect(page.locator('#scount')).toHaveText('2 / 10');
  ...
  test('keyboard shortcuts do not fire while typing in the Ask text field', async ({ page }) => {
    ...
    await page.fill('#vp-input', 'what is a dangling pointer?'); // contains a space char
    await expect(page.locator('#scount')).toHaveText('1 / 10'); // unchanged — space didn't trigger play
  });
